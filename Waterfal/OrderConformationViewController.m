...
- (void)viewDidLoad {
    ...
    _modifyOutlet.hidden = YES;
    _paynowOutlet.hidden = YES;
...
- (IBAction)conformOrderBtn:(id)sender {
    ...
    _conformOrderOutlet.hidden = YES;
    _modifyOutlet.hidden = NO;
    _paynowOutlet.hidden = NO;
    
    
    
    _detailsTxt.text = @"Your credit limit does not cover the Grand Total. Kindly modify your order or choose Pay Now option.";
            ...
            for (NSDictionary *dat in data){
             ...
             dispatch_async(dispatch_get_main_queue(), ^{
                ...
                self.taxesLabel.text = [NSString stringWithFormat:@"₹%@", [orderData objectForKey:@"tax_amount"]];
                 self.grandTotalLabel.text = [NSString stringWithFormat:@"₹%@", [orderData objectForKey:@"grand_total"]];
                 
                 
                 
                 self.detailsTxt.text = [NSString stringWithFormat:@"Payment Rs.%@ must be made with in %@", [orderData objectForKey:@"grand_total"], [NSString stringWithFormat:@"%@", [orderData objectForKey:@"order_date"]]];
...
- (IBAction)payNowBtn:(id)sender {
    ...
    PaymentViewController *go = [self.storyboard instantiateViewControllerWithIdentifier:@"paymentView"];
